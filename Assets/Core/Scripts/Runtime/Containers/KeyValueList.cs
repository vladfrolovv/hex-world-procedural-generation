...
namespace Core.Runtime.Containers
{
    ...
    {
        [SerializeField]
        protected TKey[] _serializedKeys;

        [SerializeField]
        private TValue[] _serializedValues;

        private readonly IEqualityComparer<TKey> _equalityComparer;
        protected readonly List<KeyValuePair<TKey, TValue>> Pairs = new List<KeyValuePair<TKey, TValue>>();
        ...
        void ISerializationCallbackReceiver.OnBeforeSerialize()
        {
            ...
            {
                _serializedKeys = new TKey[Pairs.Count];
                _serializedValues = new TValue[Pairs.Count];
        ...
        void ISerializationCallbackReceiver.OnAfterDeserialize()
        {
            if (_serializedKeys != null &&
                _serializedValues != null)
            {
                int keysCount = _serializedKeys.Length;
                int valuesCount = _serializedValues.Length;
                int length = keysCount;

                if (keysCount < valuesCount)
                {
                    Debug.Log($"Key for value '{_serializedValues[length]}' is lost.");
                }
                else if (valuesCount < keysCount)
                {
                    length = valuesCount;
                    Debug.Log($"Value for key '{_serializedKeys[length]}' is lost.");
                }
                ...
                {
                    Pairs.Add(new KeyValuePair<TKey, TValue>(_serializedKeys[i], _serializedValues[i]));
                }
            }
        ...
        private bool Equals(TKey first, TKey second)
        {
            ...
            return EqualityComparer<TKey>.Default.Equals(first, second);
        }
    }
